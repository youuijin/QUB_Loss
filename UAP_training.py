...
parser.add_argument('--uap_eps', type=float, default=10., help='uap step size')
...
log_name = f'{method}(eps{args.eps}_uap{args.uap_eps})_{args.loss}_lr{args.lr}_{cur}'
if args.loss == 'QUB':
    log_name = f'{method}(eps{args.eps}_uap{args.uap_eps}_lamb{args.lamb})_{args.loss}(K{args.K})_lr{args.lr}_{cur}'
...
momentum = torch.zeros((args.uap_num, *image_shape)).to(device)
feature_layer = FeatureLayer(uap_num=args.uap_num, class_num=n_way).to(device)
opt_feature_layer = torch.optim.Adam(feature_layer.parameters(), lr=0.001)
uaps = torch.zeros((args.uap_num, *image_shape)).uniform_(-args.uap_eps, args.uap_eps).to(device)
MSE_fn = torch.nn.MSELoss()

# Train Attack & Test Attack
attack = UAPAttack(model, feature_layer, opt_feature_layer, uaps, momentum, eps=args.eps, uap_eps=args.uap_eps, mean=norm_mean, std=norm_std, device=device)
...
def train(epoch):
    ...
    for inputs, targets in train_loader:
        ...
        adv_inputs, ori_adv_output = attack.perturb(inputs, targets)

        adv_outputs = model(adv_inputs)
        # loss = F.cross_entropy(adv_outputs, targets) + args.lamb * MSE_fn(adv_outputs.float(), ori_adv_output.float())
        ...
        if args.loss == 'CE':
            ...
            loss = upper_loss.mean() + args.lamb * MSE_fn(adv_outputs.float(), ori_adv_output.float())
